...
if ! grep -Fq 'INSTALLER_SKIP_UPGRADE=1 timeout 25m ./install_my_programs_unix --skip-ml --skip-fonts' "${workflow}"; then
	echo "Golden path workflow missing INSTALLER_SKIP_UPGRADE invocation"
	exit 1
fi
